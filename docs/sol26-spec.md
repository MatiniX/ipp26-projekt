...
## 1.2.11 Reakce na zprávu – rozlišování metod a instančních atributů....... 9

## 1.2.12 Příklady................................... 11

## 2 Vestavěné třídy SOL26 13

## 3 Přehled chybových návratových kódů 16

## 4 Popis XML reprezentace programu 17
...
### Rozsáhlejší příklad programu je uveden níže v sekci1.2.12.
...
### uzávěry (angl. closures), v sekci1.2.12je uveden příklad. Při pokusu o přiřazení do formálního
...
### Dále je dostupné klíčové slovo super , které odkazuje na stejný objekt jako self , ale při použití na

### místě příjemce zprávy upravuje, kde se bude hledat metoda odpovídající selektoru. Metoda bude

### nejprve vyhledávána až v přímé nadtřídě třídy, v jejíž metodě je zaslání zprávy realizováno, a pak

### postupně hledá v dalších nadtřídách (viz také sekci1.2.11). Díky tomu je možné vynutit provedení

### původní metody v případě její redefinice.
...
### žádná, jedna či více instančních metod.

### Invokací konstruktoru vzniká instance třídy (čili konkrétní objekt). Té je možné zasílat zprávy, kte-

### rým instance rozumí , tj. vlastní třída nebo nadtřída definuje metodu, která na zprávu reaguje, nebo

### odpovídající instanční atribut. Pokud zaslanou zprávu třída nepodporuje (tj. neexistuje metoda ne-

### bo instanční atribut odpovídající selektoru), program končí s chybou 51 , že instance nerozuměla

### zprávě. Podrobněji je toto chování vysvětleno v sekci1.2.11.
...
### doprava). Jako výsledek metody je navrácen výsledek bloku, který je tělem invokované metody.
...
### pak postupně v jeho předcích (viz sekci1.2.11).
...
### Uživatelsky definovaný instanční atribut není deklarován ve třídě^5 , ale je vytvářen a inicializován

### programově (za běhu) pomocí jednoparametrického selektoru^6. Výsledkem inicializační/nasta-

### vovací zprávy instančního atributu je odkaz na instanci, ve které byl atribut nastaven^7. Přístup

### k hodnotě dříve definovaného instančního atributu pak provádíme bezparametrickou zprávou se

### selektorem odpovídajícím identifikátoru atributu.

### Protože jsou instanční atributy vázány na konkrétní objekt (instanci), nemá smysl (v kontextu dě-

### dičnosti) uvažovat u nich pojem redefinice (na rozdíl od metod). Způsob řešení kolizí jmen mezi

### metodami a atributy je podrobně vysvětlen v následující podsekci (zjednodušeně: obvykle platí,

### že nelze vytvořit atribut, jehož „čtecí“ bezparametrický selektor by kolidoval se jménem metody).
...
t :=self **value** :nil.
"'r', 'e' i't' nyníukazují na self"

```
"CHYBA 54!Třída definuje metodufoo, jméno nelze použít jakoatribut."
_ :=self foo: 10.
"atributy lzevytvářet na libovolných instancích"
i := 12.
_ := i someAttr: 'hello'.
]
```
...
### vanému instančnímu atributu vede na interpretační chybu 51 (viz podsekci1.2.11).
...
### všech jejích podtřídách. Chování třídních metod není možné uživatelsky měnit, metody nelze re-

### definovat (angl. override ). Vestavěná třída String má nad rámec následujících dvou konstruktorů

### ještě další třídní metodu (konstruktor) read (viz sekci 2 ).
...
(^7) Čili reference na příjemce nastavovací zprávy.
...
### 1.2.11 Reakce na zprávu – rozlišování metod a instančních atributů

### Upozornění: Tato sekce byla do zadání dodatečně přidána 27.3. na základě nahlášené ne-

### konzistence mezi textem a příkladem. Většina zde uvedených informací už dříve vyplývala

### z předchozí verze zadání; zmatečné však bylo vysvětlení, jak se mají kontrolovat kolize metod

### a atributů. Proto nebudeme testovat žádné složitější případy kolizí jmen atributů a metod.

### Více informací najdete na fóru.

### Instance (objekt) reaguje na přijatou zprávu v zásadě tak, že nejprve hledá odpovídající metodu

### ve vlastní třídě příjemce (v případě super se hledá v nadtřídě třídy, ve které bylo super použito)

### a pokud se to nezdaří, může dojít k nastavení nebo čtení instančního atributu.

### Chování při nastavování instančních atributů a přístupu k nim se však z důvodu správného řešení

### kolizí s metodami liší v závislosti na tom, zda je na místě příjemce zprávy použito self , super ,

### nebo běžná reference^8.

### Pro pochopení popisu níže je nutné si uvědomit, že instanční atribut je „kus informace přiřazený

### ke konkrétnímu objektu“ (instanci), zatímco metody se hledají ve vlastní třídě instance (tj. ve třídě,

### podle které byla vytvořena) a poté případně v jejích nadtřídách (v řetězci dědičnosti). Způsob ře-

### šení přijaté zprávy pak instance řeší níže popsanou kombinací hledání metody v řetězci dědičnosti

### a mezi svými atributy.

(^8) Běžnou referencí se rozumí libovolný výraz na místě příjemce zprávy, který není jen klíčovým slovem **self** nebo

##### super (tj. např. proměnná nebo složitější výraz, vč. případů, kdy je self nejprve přiřazeno do proměnné).

### Příjemcem je běžná reference:

### 1. Bezparametrický selektor: Nejprve se hledá metoda s tímto selektorem, a to ve vlastní třídě

### příjemce , pak případně postupně v nadtřídách. Pokud se nikde nenajde, v příjemci se hledá

### atribut se jménem odpovídajícím selektoru. Pokud neexistuje, nastává chyba 51.

### 2. Jednoparametrický selektor: Nejprve se hledá metoda s tímto selektorem, a to ve vlastní

### třídě příjemce , pak případně postupně v nadtřídách. Pokud se nikde nenajde, příjemce se

### pokusí vytvořit atribut. To je možné jen tehdy, pokud nikde v řetězci dědičnosti (který začí-

### ná vlastní třídou příjemce) neexistuje odpovídající bezparametrický selektor (jinak nastává

### chyba kolize s metodou 54 )!

### 3. Dvou- a víceparametrický selektor: Hledá se metoda s tímto selektorem – ve vlastní třídě

### příjemce, pak případně postupně v nadtřídách. Pokud se nikde nenajde, nastává chyba 51.

### Příklad: Viz soubor1_2_11_reakce_na_zpravu_A.sol26v git repozitáři k projektu

### (adresářexamples/sol).

### Příjemcem jeself:

### 1. Bezparametrický selektor: stejně jako v případě výše.

### 2. Jednoparametrický selektor: Nejprve se hledá metoda s tímto selektorem, a to ve vlastní

### třídě příjemce , pak případně postupně v nadtřídách. Pokud se nikde nenajde, instance se

### pokusí vytvořit atribut. To je možné jen tehdy, pokud ve třídě, ve které jeselfpoužito ,

### ani v jejíchnadtřídáchneexistujeodpovídajícíbezparametrickýselektor (jinak nastáváchyba

### kolize s metodou 54 )!

### 3. Dvou- a víceparametrický selektor: stejně jako v případě výše.

### Příjemcem jesuper:

### 1. Bezparametrický selektor: Nejprve se hledá metoda s tímto selektorem, a to v nadtřídě

### třídy, ve které jesuperpoužito , pak případně postupně v jejích nadtřídách. Pokud se nikde

### nenajde, v příjemci se hledá atribut se jménem odpovídajícím selektoru. Pokud neexistuje,

### nastává chyba 51.

### 2. Jednoparametrický selektor: Nejprve se hledá metoda s tímto selektorem, a to v nadtřídě

### třídy, ve které jesuperpoužito , pak případně postupně v jejích nadtřídách. Pokud se nikde

### nenajde, instance se pokusí vytvořit atribut. To je možné jen tehdy, pokud v nadtřídě třídy,

### ve které jesuperpoužito , ani v jejích nadtřídách neexistuje odpovídající bezparametrický

### selektor (jinak nastává chyba kolize s metodou 54 )!

### 3. Dvou- a víceparametrický selektor: obdobně jako v případech výše, ale metoda se začíná

### hledat až v nadtřídě třídy, ve které je super použito.

### Může se tedy např. stát, že třída definuje instanční metodu, která překryje instanční atribut vzniklý

### v nějaké metodě její nadtřídy. V tomto případě lze k instančnímu atributu přistoupit pomocí super.

### Příklad: Viz další soubory1*2_11_reakce_na_zpravu*[...].sol26v git repozitáři

### k projektu (adresářexamples/sol).
...
### Poznámka: Další příklady najdete v git repozitáři k projektu ve složceexamples.
...
[ |
"Zašle zprávu 'compute:and:and:' soběsamému -selektor se třemi arg."
...
"Změní hodnotu,na kterou ukazujex (projeví se iv uzávěrubloku, ale
tady to ještě nicneovlivní)."
x := 7.
...
"Tatozměna x se projevíi vuzávěrubloku."
x := 9.
"Zde metodakončí,ale proměnná x(nynís hodnotou 9) stále „žije“ v
uzávěru bloku b. Následující příkaz způsobí, že giveObjectWithBlock
vrátí vytvořený objekt zjako svounávratovouhodnotu."
\_ := z.
]
...
## ci Nil , která je totožná s globálně dostupným objektem nil^9. Třídní zprávy new a from: se pro
...
(^9) Není tedy možné získat dvě instance **Nil** , pro které by **identicalTo:** vrátila **false**.
...
### divBy: Celočíselné dělení kompatibilní s implementačním jazykem interpretu^10. Dělení nulou
...
### timesRepeat: Jako argument očekává instanci, která rozumí zprávě value:^11. Pokud (a jen teh-
...
(^10) https://www.php.net/manual/en/function.intdiv.php
(^11) Např. blok s jedním parametrem.
...
## 42 neočekávaná struktura XML (např. špatné zanoření elementů, špatná hodnota atributu

## language, chybějící povinné atributy, špatné hodnoty atributu order , apod.)
...
## Za povinnou XML hlavičkou^12 následuje kořenový element program s povinným textovým atri-

## butemlanguageshodnotou "SOL26" apřípadněnepovinnýmtextovýmatributemdescription

## s krátkým popisem programu.
...
(^12) Tradiční XML hlavička včetně verze a kódování je<?xml version="1.0" encoding="UTF-8" ?>
...
## 2026-02-27: Oprava příkladu na str. 12.

## 2026-03-06: Oprava komentáře v prvním příkladu v sekci1.2.12. Oprava syntaxe v příkladu

## v sekci1.2.8. Oprava překlepu ve větě v sekciKonstruktory.

## 2026-03-27: Rozšíření komentářů v příkladu na str. 12. Přidání celé sekce1.2.11a související
